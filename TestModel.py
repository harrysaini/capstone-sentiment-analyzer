...
reviews = data.Text.values
labels = np.array([1 if s>3 else 0 for s in data.Score.values])
...
with tf.Session() as sess:
    ...
    for ii, x  in enumerate(get_batches(features , batch_size), 1):
        feed = {model.inputs_: x,
                model.keep_prob : 0.8}
        batch_pred = sess.run([model.labels_], feed_dict=feed)
